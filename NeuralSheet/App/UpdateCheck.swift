...
/// is too. An empty or unreadable response is a silent no-op, as it was: the check is a courtesy,
/// not a feature the app waits on.
enum UpdateCheck {
    ...
    static func run(for model: AppModel, explicit: Bool) {
        Task { @MainActor [weak model] in
            guard let tag = await latestTag() else { return }
